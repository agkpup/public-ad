...
const mutations = {
    matchFound(state,data){
        state.filterCity=data;
    }
}
const actions = {
    searchCities(context,data){
        if(data.length>0){
            const matches = state.AllCity.filter((e)=>{
                const regex = RegExp(`^${data}`,'gi');
                return e.name.match(regex);
            })
            if(matches.length<10){
                context.commit('matchFound',matches);
            }else{
             context.commit([{ "name": "found more than 10 result"}])  
            }
        }else{
            context.commit('matchFound',[]);
        }
},
